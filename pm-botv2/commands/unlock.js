...
module.exports.run = async (client, message, args) => {
    ...
    if(args.length <= 0)
    {
        message.channel.send('specify: channel or server');
        return;
    }
    ...
    if(args[0] == "server")
    {
        ...
        for (const channel of channels.values()) 
        {
            if(channel.type == "GUILD_CATEGORY")
            {
                ...
                {
                    channel.permissionOverwrites.edit(role.id, { SEND_MESSAGES: true });
                }
            }
        }
        ...
        message.channel.send(
        {
            ...
            [
                new Discord.MessageEmbed()
                    .setColor('RANDOM')
                    .setDescription(`Successfully unlocked the discord!`)
                    .setTitle('lock')
                    .setTimestamp(new Date)
                    .setFooter('plusminus','https://cdn.discordapp.com/attachments/874787200099831818/874790186821746728/plusminus.png')
            ]
    ...
    {
        message.channel.permissionOverwrites.edit(role.id, { SEND_MESSAGES: true });
        message.channel.send(
        {
            embeds: 
            [
